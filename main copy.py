...
def get_news_headline():
    ...
    try:
        ...
        if 'data' not in news_data or not news_data['data']:
            logging.error(f"Unexpected API response: {news_data}")
            print(f"Unexpected API response: {news_data}")
            return "News headline unavailable"
        
        headline = news_data['data'][0]['title']
        logging.info(f"News headline fetched successfully: {headline}")
        print(f"News headline fetched successfully: {headline}")
        return headline
    except Exception as e:
        logging.error(f"Error fetching news headline: {e}")
        print(f"Error fetching news headline: {e}")
        return "News headline unavailable"
